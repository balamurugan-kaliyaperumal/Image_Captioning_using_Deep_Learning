...
# Define TransformerDecoder and PositionalEncoding classes
...
linear_proj_layer = nn.Linear(768, 512)
...
def generate_caption_sampling(image_feature, max_length=80, penalty_steps=5, min_length=15):
    ...
    for step in range(max_length - 1):
        ...
        for i in range(vocab_size):
            if i in recent_ids:
                adjusted_logits[0, i] -= 15.0  # Stronger penalty for recent repetition
            elif i in used_ids:
                adjusted_logits[0, i] -= 3.0   # Stronger penalty for earlier use
            if i == 2:
                adjusted_logits[0, i] -= 25.0 * (min_length - len(caption_ids)) / min_length if len(caption_ids) < min_length else 0.0
            if i == 3:
                adjusted_logits[0, i] -= 2.0
        
        adjusted_logits /= 0.005  # Very sharp predictions
        probs = tf.nn.softmax(adjusted_logits).numpy()
        
        top_k = 300  # Wider pool
...
def generate_caption_beam_search(image_feature, max_length=80, beam_width=20):
    image_feature = tf.expand_dims(image_feature, 0)
    start_seq = [[1], 0.0]
    ...
    for step in range(max_length - 1):
        ...
        for seq, score in candidates:
            ...
            for i in range(beam_width):
                ...
                length_penalty = 0.7 * (len(new_seq) - 1) / max_length  # Stronger penalty for short sequences
                repetition_penalty = sum(1 for prev_id in seq if prev_id == new_seq[-1]) * 10.0  # Stronger penalty for repetition
                new_score = score + np.log(top_k_probs[i].numpy() + 1e-10) - length_penalty - repetition_penalty
...
@app.route('/')
def home():
    return render_template('index.html')  # Ensure this matches your file name
...
def generate_caption():
    ...
    if file and allowed_file(file.filename):
        ...
        return jsonify({
            "caption": caption_sampling,
            "caption_beam": caption_beam,
            "image_path": "/" + filepath,
